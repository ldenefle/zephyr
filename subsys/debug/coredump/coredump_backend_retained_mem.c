...
static int write_to_mem(uint8_t *buf, size_t buflen)
{
	...
	/* If we succesfully wrote to retained memory, increment our written size */
	if (!ret) {
		written += buflen;
	}
...
static int cmd_coredump_error_get(const struct shell *shell, size_t argc, char **argv)
{
	ARG_UNUSED(argc);
	ARG_UNUSED(argv);
...
static int cmd_coredump_error_clear(const struct shell *shell, size_t argc, char **argv)
{
	error = 0;
